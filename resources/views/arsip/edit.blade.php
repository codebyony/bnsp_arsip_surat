...
<div class="content-wrapper" style="padding-left: 1%; background-color: white;">
	<section class="content-header" style="margin-bottom: -1%; padding-top: 4%; background-image: url('{{ asset('dist/img/peta.png') }}'); background-repeat: no-repeat; background-position: right bottom;">
		...
		<h4 style="padding-bottom: 6%; margin-top:1%">
			Edit surat yang telah terbit pada form ini untuk diarsipkan. <br/> Catatan:
			<ul>
	...
	<section class="content">
		<div class="box box-danger">
            <!-- form start -->
            <form action="/update/{{ $item->nomor_surat }}" method="POST" class="form-horizontal" style="margin-top: 3%;" enctype="multipart/form-data">@csrf
				@method('PUT')
              <div class="box-body">
                <div class="form-group">
                  ...
                  <div class="col-sm-10 @error('nomor_surat') has-error @enderror">
                    <input type="text" class="form-control" placeholder="Masukkan Nomor Surat" name="nomor_surat" value="{{ $item->nomor_surat }}" disabled>
					@error('nomor_surat')
					...
					<div class="col-md-10">
						<select class="form-control" name="id_kategori" disabled>
							@foreach ($kategori as $k)
								<option value="{{ $k->id }}" {{ ($k->id == $item->id_kategori)?'selected':'' }}>{{ $k->nama_kategori }}</option>
							@endforeach
						</select>
					</div>
				</div>
                <div class="form-group">
					...
					<div class="col-sm-10 @error('judul') has-error @enderror">
					  <input type="text" class="form-control" placeholder="Masukkan Judul" name="judul" value="{{ $item->judul }}" disabled>
					  @error('judul')
						<span class="help-block">{{ $message }}</span>
					@enderror
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10 @error('file_surat') has-error @enderror">
						<input type="file" class="form-control" accept="application/pdf" name="file_surat" value="{{ public_path().$item->file_surat }}"/>
						@error('file_surat')
						<span class="help-block">{{ $message }}</span>
					@enderror
					</div>
              ...
              <div class="box-footer">
                <a href="/{{ $item->nomor_surat }}" class="btn btn-default">Kembali</a>
